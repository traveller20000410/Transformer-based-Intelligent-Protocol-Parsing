...
DEFAULT_I2C_CONFIG = {
        ...
        'rise_time_factor':0.1, 'fall_time_factor':0.1,
...
class RealisticI2CSignalGenerator:
    ...
    def generate_i2c_transaction(self):
        ...
        # --- 生成 ---
        # NEW: 初始化SDA电平状态。事务开始前, SDA为高电平(IDLE)
        ...
        if addr_bits == 7:
            ...
            if not simulate_addr_nack:
                low = slave & 0xFF
                s, d, l, current_sda_level = self._generate_byte(low, current_sda_level, LABEL_MAP['ADDR_10_LOW_0'],
                                                                 LABEL_MAP['ADDR_10_LOW_1'])
                scl_seq.append(s);
                sda_seq.append(d);
                lbl_seq.append(l)

                s, d, l, current_sda_level = self._generate_ack_nack(current_sda_level,
                                                                     simulate_addr_nack)  # Can also NACK here
                scl_seq.append(s);
                sda_seq.append(d);
                lbl_seq.append(l)
                if simulate_addr_nack: addr_nacked = True
        ...
        return final_scl, final_sda, final_lbl

    def generate_i2c_datasets(self, num_datasets=100, samples_per_dataset=10000):
        all_scl, all_sda, all_lbl = [], [], []
        for _ in range(num_datasets):
            sl, sd, lb = np.array([]), np.array([]), np.array([])
            while len(sl) < samples_per_dataset:
                s, d, l = self.generate_i2c_transaction()
                sl = np.append(sl, s); sd = np.append(sd, d); lb = np.append(lb, l)
                idle = np.random.randint(100,5000)
                sl = np.append(sl, np.full(idle, self.voltage_high))
                sd = np.append(sd, np.full(idle, self.voltage_high))
                lb = np.append(lb, np.full(idle, LABEL_MAP['IDLE']))
            all_scl.append(sl[:samples_per_dataset])
            all_sda.append(sd[:samples_per_dataset])
            all_lbl.append(lb[:samples_per_dataset])
        # return all_scl, all_sda, all_lbl

        protocols_dataset = np.stack((all_scl, all_sda), axis=-1)
        return protocols_dataset, all_lbl
    ...
    def save_dataset(self, scl, sda, labels, base_dir="../dataset/i2c_dataset", prefix="i2c"):
...
if __name__ == '__main__':
    ...
    all_scl, all_sda, all_lbl = gen.generate_i2c_datasets(num_datasets=10, samples_per_dataset=10000)
    for i, (s,d,l) in enumerate(zip(all_scl, all_sda, all_lbl)):
        gen.save_dataset(s, d, l, prefix=f"i2c")
    print("Done.")
